...
| `not_qualified` | `sensitive_industry` | Ecuador conflict-of-interest = "Sí" |
...
| **UC-EC2** | Conflict of interest = "Sí" (sensitive industry) | `not_qualified` / `sensitive_industry`, right after that answer |
...
| **UC-EC5** | Every screening + NSE button advances (no loop) | `conflictOfInterest` + the 8 INEC NSE buttons each advance `survey_question_index` |
| **UC-EC6** | Lead on the generic (CAM) WhatsApp number | NOT scoped; country question IS asked |
| **UC-MX1** | Lead on the México number, full survey | scoped to México; estado/municipio validated; reaches a decision |
