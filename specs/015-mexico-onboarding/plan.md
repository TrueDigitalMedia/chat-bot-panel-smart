...
| V. Country-Scoped Recruitment Configuration | PASS — Mexico is added as one `CountryConfig` (`mexico.ts`) + two data files. **T041 self-review** swept `src/**`: the only new `country === 'México'` check is the sanctioned one in `getCountryConfig()`; the `cam-nse-catalog.ts` `canonicalCountry` map gains `mexico`/`mx`/`estados unidos mexicanos → 'México'` (a label→canonical-name data entry, same category as the existing Ecuador/CAM entries, documented in `docs/countries.md`); `survey-questions.ts` gains a `country:México` picker button (data). Everything else — admin quota/dashboard, quota validation, sync, phone re-validation, the Q5-skip — flows through the config. In fact this feature **removed** a latent hardcoded assumption: `phase-1.ts`'s Q5-skip was `if (finalIdx === 5)` unconditionally; it now honours `geoHierarchy.neighborhoodLabel` (fixing a bug that also silently dropped Ecuador's manual-geo parroquia step). CAM **and Ecuador** regression suites prove other markets unchanged (the only CAM snapshot diff is the new picker button). |
