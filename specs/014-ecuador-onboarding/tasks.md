...
- [X] T036 [P] [US4] Add an Ecuador quota-config seed/fixture (`src/lib/db/seed/ecuador-quota-example.ts` or a test fixture) with `quota_targets` rows (`country='Ecuador'`, nse ∈ {AB,C,D/E}, plus edad/integrantes) and `quota_region_caps` rows for the 12 regions
  - `src/lib/db/seed/ecuador-quota-example.ts` + `npm run db:seed:ecuador-quota-example`: seeds all 12 regions × (3 NSE levels + 3 edad + 3 integrantes) open cells + a `quota_region_caps` row per region (`capCount: null`). Idempotent (upsert + conflict-tolerant caps). Uses the Phase-7-fixed `upsertQuotaTarget`/`createRegionCap` — so it's also a working proof the Ecuador quota-write path validates.
- [X] T037 [US4] Verify `checkQuotaAvailability` needs no code change — add `tests/unit/quota-ecuador.test.ts` covering: NSE-dimension match, region-cap block, and pregnancy/baby exception attribution for `country='Ecuador'`
  - 8 cases, same harness as `quota-check.test.ts`. Confirmed no code change needed — `checkQuotaAvailability` was already country-generic (spec 011).
- [X] T038 [US4] Ensure `panel-smart` / TDM sync snapshot includes `country`, `nse_region`, `nse_points`, and NSE `level` for Ecuador leads (extend the synced-answers builder in `src/lib/panel-smart/` / `src/lib/tdm-registration/` if the level/points aren't already in the diff set)
  - `country`/`nse_region`/`level`(`quota_segment`) were already synced generically. Added: `nse_points` push in `sync.ts` (Ecuador leads carry no `leads.score`, so the point total would otherwise never reach TDM), the missing `Ecuador: 'EC'` entry in `tdm-registration/country-codes.ts` (was silently sending `pais_codigo: null` for every Ecuador registration request), and the 3 migration-0029 columns (`conflictOfInterest`/`scoringAnswersJson`/`nsePoints`) to the `SurveyProfile` type. Covered by new cases in `sync.test.ts` + `build-request.test.ts`.
- [X] T039 [US4] E2E test `tests/e2e/ecuador-onboarding.spec.ts` (part 4) — open target → `lead_status='link_sent'` + Phase 2 starts; cap reached → `quota_exhausted`; `is_pregnant=true` + cap reached → `link_sent`; accepted lead's sync record shows `country='Ecuador'` + region + level
  - Shallow-smoke per this suite's convention: the 3 scenarios each seed the Ecuador quota config against the live DB (real exercise of the Phase-7 write path) then drive the full survey→quota-decision journey, asserting the webhook accepts every turn. **The DB-decision assertions (`link_sent`/`quota_exhausted`) can't run here** — the webhook's outbound Telegram sends 400 "chat not found" for a synthetic chat id and abort each turn mid-processing (the structural reason every e2e spec in this repo is a webhook-status smoke test). Those exact outcomes are asserted in `tests/unit/quota-ecuador.test.ts` (T037) and the sync-payload tests (T038); the paced-webhook DB-verification approach and why it can't work is documented in the spec file.
...
- [X] T043 [P] [US5] E2E/integration test `tests/e2e/admin-ecuador-quotas.spec.ts` — select Ecuador in the quota screen, region dropdown = 12 Ecuador regions, NSE = {AB,C,D/E}, create + persist a target and a region cap; leads filter by Ecuador
  - Spec written (3 tests, real browser interaction against a live dev server + admin login, mirroring `admin-login.spec.ts`'s convention) — **run and passing** (Chromium r1228 finished downloading after the initial attempt hit a stale r1223 cache). Also fixed the dashboard test's selector to target by accessible label (`getByLabel`) instead of DOM position — the admin sidebar's own logout `<form>` precedes the filters form in document order. Full e2e suite re-run afterward: 31/33 pass, including `admin-login.spec.ts` (4/4, previously blocked by the same missing-binary gap) and the two pre-existing `web-chat.spec.ts` copy-drift failures unrelated to this feature. Also covered by 8 unit-test cases in `tests/unit/quota-targets.test.ts` / `quota-region-caps.test.ts`.
