...
import { syncLeadPhase1Complete, syncLeadFichaHogarComplete, syncLeadFichaHogarDiscarded } from './sync'
...
describe('syncLeadPhase1Complete', () => {
  beforeEach(() => {
    vi.resetAllMocks()
  ...
  it('no-ops and returns true immediately when sync is disabled, without touching the pool', async () => {
    ...
    const result = await syncLeadPhase1Complete('lead-1', 'corr-1')
  ...
  it('no-ops and returns true when the lead already has a tdmLeadId (idempotency guard)', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(true)
    dbMock.select.mockReturnValueOnce(selectChain([{ ...LEAD_ROW, tdmLeadId: 999 }]))
  ...
  it('inserts the built row, writes tdmLeadId back, and logs success', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(true)
    dbMock.select
    ...
    expect(logCall).toHaveBeenCalledWith(
      expect.objectContaining({ leadId: 'lead-1', callType: 'tdm_mysql_sync_phase1', correlationId: 'corr-1' }),
    )
    const logCallArgs = vi.mocked(logCall).mock.calls[0][0]
    expect(logCallArgs.error).toBeUndefined()
  })

  it('returns false and logs the error without throwing when the MySQL insert fails', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(true)
    dbMock.select
      .mockReturnValueOnce(selectChain([LEAD_ROW]))
      .mockReturnValueOnce(selectChain([PROFILE_ROW]))
  ...
  it('returns false without throwing when the lead is not found', async () => {
    ...
    const result = await syncLeadPhase1Complete('missing-lead', 'corr-1')
...
describe('syncLeadFichaHogarComplete', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    vi.mocked(logCall).mockResolvedValue(undefined)
  })

  it('no-ops and returns true immediately when sync is disabled', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(false)

    const result = await syncLeadFichaHogarComplete('lead-1', 'corr-1', 'resumen')

    expect(result).toBe(true)
    expect(poolExecute).not.toHaveBeenCalled()
  })

  it('UPDATEs the existing TDM row when tdmLeadId is already set', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(true)
    dbMock.select
      .mockReturnValueOnce(selectChain([{ ...LEAD_ROW, tdmLeadId: 42 }]))
      .mockReturnValueOnce(selectChain([PROFILE_ROW]))
      .mockReturnValueOnce(selectChain([FICHA_HOGAR_ROW]))
    dbMock.update.mockReturnValue(updateChain())

    const result = await syncLeadFichaHogarComplete('lead-1', 'corr-1', 'resumen del panelista')
  ...
  it('falls back to INSERT and writes the new id back when tdmLeadId is null', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(true)
    dbMock.select
      .mockReturnValueOnce(selectChain([{ ...LEAD_ROW, tdmLeadId: null }]))
      .mockReturnValueOnce(selectChain([PROFILE_ROW]))
      .mockReturnValueOnce(selectChain([FICHA_HOGAR_ROW]))
    poolExecute.mockResolvedValue([{ insertId: 77 }])
    dbMock.update.mockReturnValue(updateChain())

    const result = await syncLeadFichaHogarComplete('lead-1', 'corr-1', null)
  ...
  it('returns false and logs without throwing when the MySQL update fails', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(true)
    dbMock.select
      .mockReturnValueOnce(selectChain([{ ...LEAD_ROW, tdmLeadId: 42 }]))
      .mockReturnValueOnce(selectChain([PROFILE_ROW]))
      .mockReturnValueOnce(selectChain([FICHA_HOGAR_ROW]))
    poolExecute.mockRejectedValue(new Error('ER_LOCK_WAIT_TIMEOUT'))
    dbMock.update.mockReturnValue(updateChain())
    ...
    expect(result).toBe(false)
    expect(logCall).toHaveBeenCalledWith(
      expect.objectContaining({ callType: 'tdm_mysql_sync_ficha_hogar', error: expect.stringContaining('ER_LOCK_WAIT_TIMEOUT') }),
    )
  })
})

describe('syncLeadFichaHogarDiscarded', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    vi.mocked(logCall).mockResolvedValue(undefined)
  })
  ...
  it('no-ops and returns true immediately when sync is disabled', async () => {
    ...
    const result = await syncLeadFichaHogarDiscarded('lead-1', 'corr-1')

    expect(result).toBe(true)
    expect(poolExecute).not.toHaveBeenCalled()
  })

  it('UPDATEs the existing TDM row with the discard status when tdmLeadId is already set', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(true)
    dbMock.select
      .mockReturnValueOnce(selectChain([{ ...LEAD_ROW, tdmLeadId: 42 }]))
      .mockReturnValueOnce(selectChain([PROFILE_ROW]))
    ...
    expect(values).toContain('ficha_hogar_descartado')
  })

  it('falls back to INSERT when tdmLeadId is null', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(true)
    dbMock.select
      .mockReturnValueOnce(selectChain([{ ...LEAD_ROW, tdmLeadId: null }]))
      .mockReturnValueOnce(selectChain([PROFILE_ROW]))
    poolExecute.mockResolvedValue([{ insertId: 88 }])
    dbMock.update.mockReturnValue(updateChain())
  ...
  it('returns false and logs without throwing when the MySQL write fails', async () => {
    isClientMysqlSyncEnabled.mockReturnValue(true)
    dbMock.select
      .mockReturnValueOnce(selectChain([{ ...LEAD_ROW, tdmLeadId: 42 }]))
      .mockReturnValueOnce(selectChain([PROFILE_ROW]))
    poolExecute.mockRejectedValue(new Error('ER_ACCESS_DENIED'))
    dbMock.update.mockReturnValue(updateChain())
    ...
    expect(result).toBe(false)
    expect(logCall).toHaveBeenCalledWith(
      expect.objectContaining({ callType: 'tdm_mysql_sync_discard', error: expect.stringContaining('ER_ACCESS_DENIED') }),
    )
  })
})
