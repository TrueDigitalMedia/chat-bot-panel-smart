...
export function FiltersForm({ countries, nseLevels, regionsByCountry }: FiltersFormProps) {
  ...
  return (
    <form className={styles.filtersForm} onSubmit={(e) => e.preventDefault()}>
      ...
      <label className={styles.filterField}>
        Nivel NSE
        <select value={searchParams.get('nseLevel') ?? ''} onChange={(e) => update('nseLevel', e.target.value)}>
          <option value="">Todos</option>
          {nseLevels.map((n) => (
